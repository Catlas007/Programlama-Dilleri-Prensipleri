...
UzayAraci* uzayAraciOlustur(const char* ad, const char* c, const char* v, Zaman t, int mesafe) {
    ...
    // Gezegen tarihine göre durumunu güncelleyecek
    a->durum = 0;  // YOLDA değil, HEDEFTE değil
    
    // Varış zamanını hesapla ve sabit bir kopyasını sakla
    a->varis_tarihi = t;
...
UzayAraci* uzayAraclariniYukle(const char* dosya_adi) {
    ...
    while (fgets(satir, sizeof(satir), f)) {
        ...
        // Araç durumunu uygun şekilde ayarla
        // Başlangıçta araç yolda değil - çıkış tarihine göre durumu belirlenecek
        yeni->durum = YOLDA;  // Varsayılan durum (düzeltilecek)
...
void uzayAraclariniGuncelle(UzayAraci* bas, const Zaman* gezegenTarih) {
    ...
    if (bas->durum != YOLDA && bas->durum != HEDEFTE && gezegenTarih != NULL) {
        ...
        if (zamanKiyasla(gezegenTarih, &bas->cikis_tarihi) >= 0) {
            bas->durum = YOLDA;
            // printf("Araç %s yola çıktı. G.Zaman: %d.%d.%d %d:00\n", 
            //        bas->ad, gezegenTarih->gun, gezegenTarih->ay, gezegenTarih->yil, gezegenTarih->saat);
        }
    }
    ...
    if (bas->durum == YOLDA) {
        ...
        if (bas->kalan_saat <= 0) {
            bas->durum = HEDEFTE;
            bas->kalan_saat = 0;
            // Varış gezegeninin zamanını kullan
            // printf("Araç %s hedefe vardı\n", bas->ad);
        }
    }
