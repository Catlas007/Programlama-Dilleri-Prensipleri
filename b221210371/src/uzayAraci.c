...
void uzayAraclariniGuncelle(UzayAraci* bas, const Zaman* gezegenTarih) {
    ...
    // Eğer araç bir gezegendeyse ve çıkış tarihi geldiyse seyahate başla
    if (bas->durum == BEKLEMEDE && gezegenTarih != NULL) {
        if (zamanKiyasla(&bas->cikis_tarihi, gezegenTarih) <= 0) {
            bas->durum = YOLDA;
            bas->kalan_saat = bas->mesafe; // Kalan saati mesafeye eşitle
        }
    }
    
    // Eğer araç yoldaysa kalan saati azalt
    if (bas->durum == YOLDA) {
        bas->kalan_saat--;
        
        // Seyahat tamamlandı mı?
        if (bas->kalan_saat <= 0) {
            bas->durum = HEDEFTE;
            bas->kalan_saat = 0;
        }
    ...
    // DİKKAT: Recursiv çağrıyı kaldırdık, bunun yerine ana fonksiyonda iteratif yaklaşım kullan
}
