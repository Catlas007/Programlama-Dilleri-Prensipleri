...
static void hesaplaVarisTarihiHedefGezegeneGore(UzayAraci* arac, GezegenNode* gezegenler) {
    ...
    // Seyahat süresini STANDART SAAT olarak hesapla
    // Her saat için standart 24 saatlik zaman ilerletme kullan
    int kalan_saat = arac->mesafe;
    
    while (kalan_saat > 0) {
        // Standart 24 saatlik zaman ilerletme kullan
        birSaatIlerle(&arac->varis_tarihi);
        kalan_saat--;
    }
    
    // Şimdi bu tarihi hedef gezegenin zaman dilimine çevir
    // Hedef gezegenin zaman dilimi ile senkronize et
    // (Bu kısım hedef gezegenin mevcut tarihi dikkate alınarak yapılabilir)
}
...
static void kisileriGuncelleYaslandir(Kisi* kisiler, UzayAraci* araclar, GezegenNode* gezegenler) {
    ...
    while (kisi) {
        ...
        // If vehicle is on a planet (not in transit), use that planet's aging factor
        ...
        // If vehicle is YOLDA (in space), use normal aging (1.0)
        
        // Age the person using kisi.c function
        kisiYaslandir(kisi, yaslanma_carpani);
        
        // Move to next person (since kisileriYaslandir only affects one person)
        // We need to call it on each person individually
        kisi = kisi->sonraki;
    }
}
...
while (node) {
        ...
        if (kisi->kalan_omur > 0) {
            ...
            while (a) {
                if (strcmp(kisi->bulundugu_arac_adi, a->ad) == 0) {
                    // DÜZELTİLDİ: İMHA araçlardaki kişiler hiçbir gezegene dahil edilmez
                    if (a->durum != IMHA) {
    ...
    // UZAY ARAÇLARI - Format ödevdeki gibi
    ...
    while (a) {
        ...
        if (a->durum == IMHA) {
            ...
            // Henüz başlamamış araçlar için mesafeyi göster
            printf("%-17d %02d.%02d.%04d\n", a->mesafe,
                   a->varis_tarihi.gun, a->varis_tarihi.ay, a->varis_tarihi.yil);
        }
...
void simulasyonuBaslat(Kisi* kisiler, UzayAraci* araclar, GezegenNode* gezegenler) {
  ...
  while (!tumAraclarHedefteVeyaImha(araclar)) {
    ...
    // 3. KİŞİLERİ YAŞLANDIR (İMHA kontolünden ÖNCE)
    kisileriGuncelleYaslandir(kisiler, araclar, gezegenler);
    
    // 4. İMHA kontrolü tekrar yap (yaşlandıktan sonra)
