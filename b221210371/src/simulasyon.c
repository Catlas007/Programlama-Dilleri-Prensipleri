...
static void hesaplaVarisTarihiHedefGezegeneGore(UzayAraci* arac, GezegenNode* gezegenler) {
    ...
    // Her saat için hedef gezegenin zaman sistemini kullanarak ilerle
    while (kalan_saat > 0) {
        // Hedef gezegenin gün uzunluğuna göre zaman ilerlet
        birSaatIlerleGezegen(&arac->varis_tarihi, varis_gezegeni->gun_saat);
        kalan_saat--;
    }
}
            ...
            if (kisi->kalan_omur > 0) {
                ...
                while (a) {
                    if (strcmp(kisi->bulundugu_arac_adi, a->ad) == 0) {
                        ...
                        // DÜZELTİLDİ: BEKLEMEDE araçlar da çıkış gezegeninde sayılır
                        if ((a->durum == HEDEFTE && strcmp(a->varis_gezegeni, g->ad) == 0) ||
                            ((a->durum == BEKLEMEDE || a->durum == 3) && strcmp(a->cikis_gezegeni, g->ad) == 0)) {
                            nufus++;
                        }
                        break;
                    }
