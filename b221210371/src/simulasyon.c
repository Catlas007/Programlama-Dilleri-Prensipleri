...
// Update time on all planets - FIXED: Account for different day lengths properly
static void gezegenleriGuncelle(GezegenNode* gezegenler) {
    ...
    while (current) {
        ...
        // Use planet-specific day length instead of fixed 24 hours
        if (g->tarih.saat >= g->gun_saat) {
            g->tarih.saat = 0;
            g->tarih.gun++;
            
            // Check if we need to advance to next month
            if (g->tarih.gun > gunSayisi(g->tarih.ay, g->tarih.yil)) {
                g->tarih.gun = 1;
                g->tarih.ay++;
