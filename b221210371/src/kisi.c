...
void kisileriYaslandir(Kisi* bas, double carpan) {
    while (bas) {
        if (bas->kalan_omur > 0) {
            double onceki_omur = bas->kalan_omur;
            bas->kalan_omur -= carpan;
            
            // 1 yıl = 8760 saat (365 gün × 24 saat)
            // Yaş, kalan ömür belirli bir yıl eşiğini geçtiğinde artmalı
            if ((int)(onceki_omur / 8760) > (int)(bas->kalan_omur / 8760)) {
                bas->yas++;
            }
        }
        bas = bas->sonraki;
